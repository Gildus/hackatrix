...
//    flightPath5.setMap(map);
    ...
    $.each(rutaHacka, function(e,p){
        
        var flightPlanCoord = new Array();
        $.each(p.r, function(ind, val){        
            var point = new google.maps.LatLng(val.x,val.y);            
            flightPlanCoord.push(point);
            
        });
        
        var rutaPath = new google.maps.Polyline({
          path: flightPlanCoord,
          geodesic: true,
          strokeColor: p.c,
          strokeOpacity: 1.0,
          strokeWeight: 2
    ...
    function placeMarkerInicio(location) {
        ...
        // Otherwise, simply update its location on the map.
            ...
            return comparador(
                ...
                markerFin.getPosition().lng(),rutaHacka);
        }
    }
    ...
    function comparador(lat1,long1,lat2,long2,pRutas) {
        var res = [];       
        $.each(pRutas, function(ind, el){            
            resComp = comp(lat1,long1,lat2,long2,el.r);
            res.push({"x":resComp, "y": el.n});
        });
